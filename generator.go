...
func (g *generator) generateServiceClient(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) *plugin.CodeGeneratorResponse_File {
    ...
    g.P(`  }`)
    g.P()
    g.P(`  void setResponseMetaDataProcessor(IResponseMetaDataProcessor processor){ this.responseMetaDataProcessor = processor; }`)
    g.P(`  IResponseMetaDataProcessor getResponseMetaDataProcessor(){ return this.responseMetaDataProcessor; }`)
    g.P()
    ...
    g.P(`      response.close();`)
    g.P(`      if(this.responseMetaDataProcessor != null){ this.responseMetaDataProcessor.process(response); }`)
    g.P(`      return r;`)
    g.P(`    } else {`)
