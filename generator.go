...
func (g *generator) generateServiceClient(file *descriptorpb.FileDescriptorProto, service *descriptorpb.ServiceDescriptorProto) *pluginpb.CodeGeneratorResponse_File {
    ...
    g.P(`    try{`)
    g.P(`       retryTimeout = Long.parseLong(System.getProperty("JAVAX_RETRY_TIMEOUT"));`)
    g.P(`    } catch(NumberFormatException e){`)
    g.P(`       retryTimeout = 500L;`)
    g.P(`    }`)
    g.P(`  }`)
    g.P()
    ...
    g.P(`      return response.readEntity(responseClass);`)
    g.P(`    } else {`)
    g.P(`      throw new javax.ws.rs.WebApplicationException(`)
    g.P(`       response.getStatus() + " " + response.getStatusInfo().getReasonPhrase() + ": " + response.readEntity(String.class));`)
    g.P(`    }`)
    g.P(`  }`)
    g.P()
    ...
    g.P(`      private final java.util.concurrent.atomic.AtomicInteger count = new java.util.concurrent.atomic.AtomicInteger(retries);`)
    g.P()
    g.P(`      @Override`)
    ...
    g.P(`        if(responseMetaDataProcessor != null){ responseMetaDataProcessor.process(response); }`)
    g.P(`        if (response.getStatusInfo().getFamily() == javax.ws.rs.core.Response.Status.Family.SUCCESSFUL) {`)
    g.P(`          R result = response.readEntity(responseClass);`)
    g.P(`          if(predicate != null){`)
    g.P(`            if (!predicate.apply(result) && count.get() > 0) {`)
    g.P(`               failed(new java.net.HttpRetryException("Retrying because predicate failed for entity: \n" + response.readEntity(String.class), 0));`)
    g.P(`            }`)
    g.P(`          }`)
    g.P(`          future.complete(result);`)
    g.P(`        } else {`)
    g.P(`          this.failed(new javax.ws.rs.WebApplicationException(`)
    g.P(`            response.getStatus() + " " + response.getStatusInfo().getReasonPhrase() + ": " + response.readEntity(String.class)));`)
    g.P(`        }`)
    g.P(`      }`)
    g.P()
    ...
    g.P(`        if (count.getAndDecrement() > 0) {`)
    g.P(`          try {`)
    g.P(`            Thread.sleep(retryTimeout);`)
