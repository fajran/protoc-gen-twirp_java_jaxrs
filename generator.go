...
func (g *generator) generateServiceClient(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) *plugin.CodeGeneratorResponse_File {
    ...
    g.P(`     if(retries < 0){throw new IllegalArgumentException("Retries count can't be less than 0");}`)
    ...
    g.P(`           if (count.getAndDecrement() < retries) {`)
    g.P(`              try {`)
    ...
    g.P(`           } else {`)
	g.P(`              throw new javax.ws.rs.WebApplicationException(throwable);`)
    g.P(`           }`)
    g.P(`        }`)
    g.P(`     });`)
    g.P(`     return future;`)
