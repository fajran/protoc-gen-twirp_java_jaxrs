...
func (g *generator) generateServiceClient(file *descriptorpb.FileDescriptorProto, service *descriptorpb.ServiceDescriptorProto) *pluginpb.CodeGeneratorResponse_File {
    ...
    for _, method := range service.Method {
        ...
        g.P(`    return call("/`, methodPath, `", request, new javax.ws.rs.core.MultivaluedHashMap<String, Object>(), `, outputType, `.class);`)
        g.P(`  }`)
        g.P()
        ...
        g.P(`    javax.ws.rs.core.MultivaluedMap<String, Object> headers,`)
        g.P(`    java.util.function.Function<`, inputType,`, `, `Boolean> predicate,`)
        g.P(`    Integer retries`)
        g.P(`  ){`)
        g.P(`    if(retries == 0){retries = 10;}`)
        ...
        g.P(`    Integer retries`)
        g.P(`  ){`)
        g.P(`    if(retries == 0 || retries == null){retries = 10;}`)
        g.P(`    return call("/`, methodPath, `", request, new javax.ws.rs.core.MultivaluedHashMap<String, Object>(), `, outputType, `.class, predicate, retries);`)
        g.P(`  }`)
        g.P()
        ...
        g.P(`  ){`)
        g.P(`    if(retries == 0 || retries == null){retries = 10;}`)
        g.P(`    return call("/`, methodPath, `", request, new javax.ws.rs.core.MultivaluedHashMap<String, Object>(), `, outputType, `.class, null, retries);`)
        g.P(`  }`)
    }
